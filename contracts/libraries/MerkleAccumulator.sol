...
library MerkleAccumulator {
    ...
    function append(MerkleAccumulatorStorage memory self, bytes32 itemHash)
        internal
        returns (MerkleTreeNodeEvent[] memory)
    {
        ...
        while (true) {
            if (level == calcNumPartials(size - 1)) {
                // -1 to counteract the size++ at top
            ...
            events[eventCount] = MerkleTreeNodeEvent({level: level, numLeaves: size - 1, hash: soFar});
            eventCount++;
        }
    ...
    function stateForExport(MerkleAccumulatorStorage memory self)
        internal
        view
        returns (uint64, bytes32, bytes32[] memory)
    {
