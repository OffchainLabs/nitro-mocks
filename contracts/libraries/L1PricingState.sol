...
library L1PricingState {
    ...
    function batchPosterTable(L1PricingStorage memory self) internal pure returns (BatchPostersTableStorage memory) {
        bytes memory tableKey = ArbosStorage(self.store.addr).openSubStorage(self.store.key, BATCH_POSTER_TABLE_KEY);
        return BatchPostersTableStorage({store: Storage({addr: self.store.addr, key: tableKey})});
    }
